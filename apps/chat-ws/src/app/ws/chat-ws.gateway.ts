...
import { ChatCursorService, ChatHistoryService, ChatRoomService } from '@eco-books/chat-core';
import { BookServiceClients } from '@eco-books/external-clients';
import { ChatCursorDto, ChatDto, ChatIncomeDto, JwtPayload } from '@eco-books/type-common';
...
export class ChatWsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  private updateCursorWithAnotherClients(userId: number, cursorDto :ChatCursorDto) {
    this.server.to(this.USER_CLIENTS_ROOM_ID_PREFIX + userId).emit('/chat/update-cursor', cursorDto);
  }
}
